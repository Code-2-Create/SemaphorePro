...
import { GET_CHAR_MAPPING, SAMPLE_NAVAL_PHRASES, SAMPLE_WORDS } from '../constants';
...
const PracticeMode: React.FC<PracticeModeProps> = ({ onSessionComplete }) => {
  ...
  const startPractice = (type: 'short' | 'long' | 'drill') => {
    ...
    if (type === 'drill') {
      ...
      if (type === 'short') {
        pool = SAMPLE_NAVAL_PHRASES.filter(p => p.split(' ').length <= 4);
      } else if (type === 'long') {
        pool = SAMPLE_NAVAL_PHRASES.filter(p => p.split(' ').length > 4);
      }
      newPhrase = pool[Math.floor(Math.random() * pool.length)] || SAMPLE_NAVAL_PHRASES[0];
    }
  ...
  useEffect(() => {
    ...
    } else if (currentIndex >= transmissionQueue.length - 1 && transmissionQueue.length > 0) {
      setIsPlaying(false);
    }
    return () => {
  ...
  const currentChar = currentIndex >= 0 && currentIndex < transmissionQueue.length ? transmissionQueue[currentIndex] : " ";
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-slate-100 w-full flex flex-col items-center relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center w-full mb-8 space-y-6 md:space-y-0">
          <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shadow-inner">
            {!isPlaying ? (
              <>
                <button onClick={() => startPractice('short')} className="px-5 py-2.5 hover:bg-white hover:shadow-sm rounded-xl transition-all text-xs font-bold text-slate-600 flex items-center space-x-2">
                  <i className="fas fa-bolt text-amber-500"></i>
                  <span>Short</span>
                </button>
                <button onClick={() => startPractice('long')} className="px-5 py-2.5 hover:bg-white hover:shadow-sm rounded-xl transition-all text-xs font-bold text-slate-600 flex items-center space-x-2">
                  <i className="fas fa-anchor text-blue-500"></i>
                  <span>Message</span>
                </button>
                <button onClick={() => startPractice('drill')} className="px-5 py-2.5 bg-blue-600 shadow-md rounded-xl transition-all text-xs font-black text-white flex items-center space-x-2">
                  <i className="fas fa-shield-halved"></i>
                  <span>40x40 Drill</span>
                </button>
              </>
            ) : (
              <button onClick={() => setIsPlaying(false)} className="px-8 py-2.5 bg-red-500 text-white shadow-lg rounded-xl transition-all text-xs font-black flex items-center space-x-2 animate-pulse">
          ...
          <div className="flex items-center space-x-6">
            <button 
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-xl border transition-all ${showHints ? 'bg-amber-50 border-amber-200 text-amber-600 shadow-sm' : 'bg-slate-50 border-slate-200 text-slate-400'}`}
            >
              <i className={`fas ${showHints ? 'fa-eye' : 'fa-eye-slash'}`}></i>
              <span className="text-[10px] font-black uppercase tracking-widest">{showHints ? 'Hints ON' : 'Hints OFF'}</span>
            </button>

            <div className="flex flex-col items-end">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Speed Multiplier</span>
              <div className="flex items-center space-x-3">
                ...
                />
                <span className="text-sm font-mono font-bold text-blue-600 w-12 text-right">{speedLevel}%</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="mt-10 flex flex-col items-center w-full max-w-sm">
             <div className="flex justify-between w-full text-[10px] uppercase font-black text-slate-400 tracking-[0.2em] mb-3">
               <span className="flex items-center space-x-2">
                 <span className={`w-2 h-2 rounded-full ${isPlaying ? 'bg-green-500 animate-ping' : 'bg-slate-300'}`}></span>
                 <span>Signal: {currentChar === '#' ? 'NUMERIC' : (currentChar === ' ' ? 'REST' : currentChar)}</span>
               </span>
               <span>{transmissionQueue.length > 0 ? `${currentIndex + 1} / ${transmissionQueue.length}` : 'STANDBY'}</span>
             </div>
             <div className="h-4 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200 p-1 shadow-inner">
                <div 
                  ...
                  style={{ width: `${transmissionQueue.length > 0 ? ((currentIndex + 1) / transmissionQueue.length) * 100 : 0}%` }}
                />
             </div>
          </div>
        </div>
      ...
      {(phrase && !isPlaying) && (
        <div className="w-full bg-white p-8 rounded-[2.5rem] shadow-2xl border border-slate-100 animate-in fade-in slide-in-from-bottom-6 duration-700">
          <div className="flex justify-between items-center mb-6">
            ...
            {isFinished && (
              <div className={`px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest ${calculateAccuracy(phrase, userInput) > 85 ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
