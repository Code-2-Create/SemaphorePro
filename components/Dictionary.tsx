...
const Dictionary: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-8 w-full max-w-6xl animate-in fade-in duration-500">
          ...
          {filteredMap.map((m) => (
            ...
            >
              <div className="w-12 h-12 flex items-center justify-center">
                 <Signalman leftPos={m.left} rightPos={m.right} size={40} animate={false} />
              </div>
        ...
        {selectedChar ? (
          <>
            <div className="flex justify-between w-full mb-4 items-center">
               <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Detail View</span>
               <button onClick={() => setSelectedChar(null)} className="text-slate-500 hover:text-white"><i className="fas fa-times"></i></button>
            </div>
            <h3 className="text-4xl font-black mb-6">
              {selectedChar === '#' ? 'Numeric Indicator' : `Signal: ${selectedChar}`}
            </h3>
            <div className="bg-white rounded-3xl p-6 mb-8 shadow-inner">
              <Signalman 
                leftPos={SEMAPHORE_MAP.find(m => m.char === selectedChar)!.left} 
                rightPos={SEMAPHORE_MAP.find(m => m.char === selectedChar)!.right} 
                size={220}
              />
            </div>
            <div className="w-full text-left space-y-4">
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="bg-slate-800 p-4 rounded-2xl text-center border border-slate-700/50">
                  <span className="block text-slate-500 font-bold uppercase mb-1">Left Arm</span>
                  <span className="font-mono text-blue-300 text-lg">Pos {SEMAPHORE_MAP.find(m => m.char === selectedChar)!.left}</span>
                </div>
                <div className="bg-slate-800 p-4 rounded-2xl text-center border border-slate-700/50">
                  <span className="block text-slate-500 font-bold uppercase mb-1">Right Arm</span>
                  <span className="font-mono text-blue-300 text-lg">Pos {SEMAPHORE_MAP.find(m => m.char === selectedChar)!.right}</span>
                </div>
              </div>
              <div className="bg-blue-600/10 p-4 rounded-2xl border border-blue-500/20">
                <p className="text-[10px] text-blue-400 font-black uppercase tracking-widest mb-2">Technical Info</p>
                <p className="text-xs text-slate-300 leading-relaxed">
                  Used in {selectedChar >= '0' && selectedChar <= '9' ? 'Numeric Mode' : 'Standard Alpha Mode'}. 
                  Arms positioned at {SEMAPHORE_MAP.find(m => m.char === selectedChar)!.left * 45}° and {SEMAPHORE_MAP.find(m => m.char === selectedChar)!.right * 45}°.
                </p>
              </div>
            </div>
          </>
          ...
          <div className="py-20 space-y-6">
            ...
            <p className="font-bold text-slate-400">Select a character from the dictionary to inspect the signal positions.</p>
          </div>
        )}
      </div>
    </div>
