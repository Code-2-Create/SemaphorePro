...
const App: React.FC = () => {
  ...
  const NavItem = ({ m, icon, label, onClick }: { m: AppMode, icon: string, label: string, onClick?: () => void }) => (
    <button 
      onClick={() => {
        setMode(m);
        if (onClick) onClick();
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <header className="bg-white/90 backdrop-blur-lg border-b border-slate-200 sticky top-0 z-50 shadow-lg">
        <div className="px-4 py-3 flex justify-between items-center">
          ...
          >
            <i className={`fas ${menuOpen ? 'fa-times' : 'fa-bars'} text-slate-700 text-xl`}></i>
          </button>
        </div>
      </header>
      ...
      >
        <div className="p-6 space-y-6 h-full overflow-y-auto">
          ...
          <nav className="space-y-3">
            <NavItem m="HOME" icon="fa-home" label="Home" onClick={() => setMenuOpen(false)} />
            <NavItem m="PRACTICE" icon="fa-bullseye" label="Practice" onClick={() => setMenuOpen(false)} />
            <NavItem m="DICTIONARY" icon="fa-book" label="Learn Symbols" onClick={() => setMenuOpen(false)} />
          </nav>
          ...
          <div className="pt-6 border-t border-slate-200">
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-5 rounded-2xl border-2 border-blue-100">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <div>
                  <p className="text-[9px] font-black text-blue-600 uppercase tracking-wider">Current Rank</p>
                  <p className="text-sm font-black text-slate-800">{getRank()}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 mt-4">
                <div className="bg-white p-3 rounded-xl">
                  <p className="text-[8px] font-black text-slate-500 uppercase">Sessions</p>
                  <p className="text-xl font-black text-blue-600">{getTotalTransmissions()}</p>
                </div>
                <div className="bg-white p-3 rounded-xl">
                  <p className="text-[8px] font-black text-slate-500 uppercase">Accuracy</p>
                  <p className="text-xl font-black text-green-600">{getAverageAccuracy()}%</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/80 backdrop-blur-sm p-5 rounded-2xl shadow-lg border border-slate-200">
                ...
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-1">Transmissions</p>
              ...
              <div className="bg-white/80 backdrop-blur-sm p-5 rounded-2xl shadow-lg border border-slate-200">
                ...
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-1">Avg Accuracy</p>
            ...
            <div className="bg-white/80 backdrop-blur-sm p-6 rounded-3xl shadow-lg border border-slate-200">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center space-x-1 bg-blue-50 px-3 py-1 rounded-lg">
                  ...
                  <span className="text-[9px] font-black text-blue-600 uppercase">Accuracy</span>
                </div>
              </div>
              ...
              <div className="h-64">
                {history.length > 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-400">
                    ...
                    <p className="font-bold text-sm text-slate-600 mb-2">Start Your First Session</p>
                    <button
                      onClick={() => setMode('PRACTICE')}
            ...
            <div className="bg-white/80 backdrop-blur-sm p-6 rounded-3xl shadow-lg border border-slate-200">
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {history.length > 0 ? history.slice(0, 8).map((session, idx) => (
                  <div key={session.id} className="p-4 bg-gradient-to-br from-slate-50 to-blue-50 rounded-xl border border-slate-100">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-[9px] text-slate-500 font-bold">{new Date(session.date).toLocaleDateString()}</span>
                    </div>
                    <p className="text-[11px] font-mono font-bold text-slate-700 line-clamp-2 uppercase">{session.phrase}</p>
                  </div>
                )) : (
                  <div className="text-center py-12">
                    ...
                    <p className="text-xs font-bold text-slate-500 mb-3">No sessions yet</p>
                    <button
                      onClick={() => setMode('PRACTICE')}
      ...
      <footer className="bg-white/80 backdrop-blur-md border-t border-slate-200 py-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 text-center space-y-4">
          <div className="flex justify-center space-x-6 text-slate-400 text-xl">
            <a href="https://github.com/Code-2-Create" target="_blank" rel="noopener noreferrer" className="hover:text-slate-900 transition-all">
              <i className="fab fa-github"></i>
            </a>
            <a href="https://www.linkedin.com/in/vinayaksen77" target="_blank" rel="noopener noreferrer" className="hover:text-blue-700 transition-all">
